...
Page({
  ...
  formSubmit: function (e) {
    ...
    var payPassword = e.detail.value.payPassword; 

    //表单验证
    function check(phoneNo, verificationCode, payPassword) {
      if (phoneNo == '') {
        wx.showToast({
          title: '请填写联系电话',
          icon: 'none',
      ...
      if (that.data.matchpwd == '') {
        ...
        if (that.data.matchpwd != payPassword) {
          wx.showToast({
            title: '两次密码输入不一致',
            icon: 'none',
            duration: 1000
    ...
    if (check(phoneNo, verificationCode, payPassword)) {
      //设置支付密码
      wx.request({
        url: app.globalData.appUrl + 'WXBuyerAccount/modifyBuyerAccountpayPassword',
        data: {
